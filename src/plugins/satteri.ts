import type { Code, Image, Nodes, Parents } from "mdast";
	...
	return () => {
		...
		return {
			...
			text(node, ctx) {
				...
				if (frontmatter) frontmatter.readingTime = readingTime.text;
			},
		};
	};
}
...
export function satteriMathBlockPlugin(): MdastPluginDefinition {
	return {
		name: "cactus-math-blocks",
		math(node): Code {
			return {
				lang: "math",
				type: "code",
